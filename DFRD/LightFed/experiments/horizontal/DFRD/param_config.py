...
def get_args():
    ...
    parser.add_argument('--model_here_level', type=int, default=5) 
    ...
    parser.add_argument('--model_split_mode', type=str, default='roll', choices=['roll', 'static', 'random', 'None'], help='method of spliting global model')

    parser.add_argument('--global_model_rate', type=float, default=MODEL_SPLIT_RATE['a'], help='size of global model')

    parser.add_argument('--weighting', default='updates', type=str, choices=['avg', 'width', 'updates', 'updates_width'], help='agg strategy')

    parser.add_argument('--scale', type=lambda s: s == 'True', default=True)

    parser.add_argument('--mask', type=lambda s: s == 'True', default=False)

    parser.add_argument('--mask_ensemble_gen', type=lambda s: s == 'True', default=True)

    parser.add_argument('--mask_ensemble_glo', type=lambda s: s == 'True', default=True)
    ...
    parser.add_argument("--gen_I", type=int, default=5, help="training number of generator")
    ...
    parser.add_argument("--beta_tran", type=float, default=1.0, help="hyper-parameter of L_tran loss")
    ...
    parser.add_argument("--lambda_", type=float, default=0.1, help="momentum for EMA generator")

    parser.add_argument("--alpha", type=float, default=0.5, help="hyper-parameter for EMA KL loss")
    ...
    parser.add_argument('--tran_strategy', type=str, default='1', choices=['1', '2', '3'], 
                        help='1, 2 and 3 denote tran strategy for DFDR, FTG, DENSE, and beta_tran=0 denotes no tran_strategy')
    
    parser.add_argument('--agg_strategy', type=str, default='1', choices=['1', '2', '3'], 
                        help='1, 2 and 3 denote tran strategy for DFDR (Dynamic averaging), FTG (Static averaging), DENSE (Naive averaging)')
    
    parser.add_argument('--save_generator', type=lambda s: s == 'True', default=False)

    parser.add_argument('--global_lr', type=float, default=0.01, help="SGD: learning rate")

    parser.add_argument("--global_I", type=int, default=1, help="training number of global model")
    ...
    parser.add_argument('--app_name', type=str, default='Fedrolex_resnet_data_free')
...
def _get_model_split_rate(args):
    if args.model_heterogeneity == False:
        reta_list = [1.0 for _ in range(args.client_num)]
    else:
        reta_list = []
