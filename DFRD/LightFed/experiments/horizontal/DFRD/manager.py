...
class ServerManager(BaseServer):
    def __init__(self, ct, args):
        ...
        self.agg_strategy = args.agg_strategy
        ...
        self.EMA_generator = generator_model_pull(self.super_params).to(self.device)
    ...
    def get_label_weights(self):
        ...
        for label in range(self.n_class):
            weights = []
            if len(self.label_counts_collect) == self.client_num:
                ...
                for user in range(self.client_num):
                    if user in self.label_counts_collect:
                        weights.append(self.label_counts_collect[user][label])
                    else:
                        weights.append(1)
    ...
    def fed_finish_client_train_step(self,
        ...
        if not self.unfinished_client_num:
            ...
            if self.step == 0:
                EMA_gen_param = formula(lambda  w: (1 - self.lambda_) * w,
                                        get_parameters(self.generator.state_dict()))
            elif self.step > 0:
                EMA_gen_param = formula(lambda EMA_w, w: self.lambda_ * EMA_w + (1 - self.lambda_) * w,
                            get_parameters(self.EMA_generator.state_dict()), get_parameters(self.generator.state_dict()))
            self.EMA_generator.load_state_dict(EMA_gen_param, strict=True)

            if self.save_generator == True:
                if self.step in [int(self.comm_round * 0.2)-1, int(self.comm_round * 0.4)-1, int(self.comm_round * 0.6)-1, int(self.comm_round * 0.8)-1, int(self.comm_round * 1.0)-1]:
                    torch.save((self.generator, self.EMA_generator), \
                       f"{os.path.dirname(__file__)}/generator/{self.data_set}_{self.noise_label_combine}_{str(self.step)}_{self.seed}.pth")
    ...
    def Update_generator(self, ):
        ...
        for _ in range(self.gen_I):
            ...
            if self.agg_strategy == '3': 
                y = self.random_choice_y(self.batch_size)
            else:
            ...
            L_fid = self.CE(L_kl_logit, gen_labels)
            ...
            L_div = self.diversity_loss(noises=mul_h, layer=gen_imgs)
            if self.noise_label_combine in ['mul', 'add', 'cat']:
        ...
        info_gen = {'comm. round': self.step, 'client_id': 'server', 'update': 'generator',
                    'LOSS_GEN': round(LOSS_GEN / self.gen_I, 4), 'L_CLS': round(L_FID / self.gen_I, 4),
                    'L_TRAN': round(L_TRAN / self.gen_I, 4), 'L_DIV': round(L_DIV / self.gen_I, 4), 'L_EY': round(L_EY / self.gen_I, 4)}
        self.server_train_test_res.update(generator_update=info_gen)
    
    def Update_global_model(self,):
        grad_False(self.generator)
        self.generator.eval()
        grad_False(self.EMA_generator)
        self.EMA_generator.eval()

        grad_True(self.model)
        self.model.train()
        LOSS_GLO = 0
        L_KL, EMA_L_KL = 0.0, 0.0
        ...
        for _ in range(self.global_I):
            ...
            L_kl = self.cal_L_kl(self.generator, z, y, gen_labels)

            if self.step == 0:
                loss_glo = L_kl
            elif self.step > 0:
                EMA_z = Variable(self.FloatTensor(np.random.normal(0, 1, (self.batch_size, self.latent_dim))))
                if self.agg_strategy == '3': 
                    EMA_y = self.random_choice_y(self.batch_size)
                else:
                    EMA_y = self.random_choice_y(self.batch_size, self.label_num)
                EMA_gen_labels = Variable(self.LongTensor(np.array(EMA_y)))
                EMA_L_kl = self.cal_L_kl(self.EMA_generator, EMA_z, EMA_y, EMA_gen_labels)
                loss_glo = L_kl + self.alpha * EMA_L_kl

            loss_glo.backward()
            self.optimizer_model.step()
            LOSS_GLO += loss_glo.detach().cpu().numpy()
            L_KL += L_kl.detach().cpu().numpy()
            try:
                EMA_L_KL += EMA_L_kl.detach().cpu().numpy()
            except:
                EMA_L_KL = 0.0

        info_global = {'comm. round': self.step, 'client_id': 'server', 'update': 'global_model',
                'LOSS_GLO': round(LOSS_GLO / self.gen_I, 4), 'L_KL': round(L_KL / self.gen_I, 4), 'EMA_L_KL': round(EMA_L_KL / self.gen_I, 4)}
    ...
    def cal_L_kl(self, generator, z, y, gen_labels):
        ...
        for client_idx, local_model in self.client_collect_list:
            ...
            if self.mask_ensemble_glo == True:
                client_logits = local_model(gen_imgs, label_list=self.label_split[client_idx])
            else:
                client_logits = local_model(gen_imgs)
